		...
		if err != nil {
			...
			if err != nil {
				if errors.Is(err, utils.ErrPhotoDoesNotExist) {
					...
					w.WriteHeader(http.StatusInternalServerError)
					ctx.Logger.WithError(err).Error("Si è verificato un errore nel controllare l'sistenza della foto.")
					return

				}
			} else {
				// La foto esiste
				// Devo controllare che l'Id passato nell'Authorization corrisponda
						...
						if authorization_type != "Bearer" || strings.TrimSpace(id1) == "" {
							...
							if err != nil {
								...
								if err != nil {
									if errors.Is(err, utils.ErrUserDoesNotExist) {
										w.WriteHeader(http.StatusUnauthorized)
										ctx.Logger.WithError(err).Error("L'authorization non corrisponde ad un user con ptofilo esistente.")
										return
									} else {
										w.WriteHeader(http.StatusInternalServerError)
									...
									if err != nil {
										...
										if strings.Split(r.Header.Get("Authorization"), " ")[1] != strings.Split(r.URL.Path, "/")[6] {
											w.WriteHeader(http.StatusUnauthorized)
											ctx.Logger.WithError(err).Error("L'user non è autorizzato a togliere like non suoi .")
											return

										} else {
											err = rt.db.UnlikePhoto(*urlid, photoid, strings.Split(r.URL.Path, "/")[6])
											if err != nil {
												w.WriteHeader(http.StatusInternalServerError)
												ctx.Logger.WithError(err).Error("Errore nelle operazioni di database.")
												return
											} else {
