					...
					if err != nil {
						...
						if err != nil {
							...
							if err != nil {
								...
								if *urlid != id1 {
									w.WriteHeader(http.StatusForbidden)
									ctx.Logger.Error("Un utente può richiedere solo la sua personale lista di ban.")
									return
								} else {
									bannedusers, err := rt.db.GetBanned(*urlid)
									if err != nil {
										w.WriteHeader(http.StatusInternalServerError)
										...
										getBannedResponseBody := getBannedResponseBody{BannedUsers: *bannedusers}
										w.Header().Set("Content-Type", "application/json")
										w.WriteHeader(http.StatusOK)
										err = json.NewEncoder(w).Encode(getBannedResponseBody)
